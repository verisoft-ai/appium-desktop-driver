...
/// Element IDs use the format "dotnet:{pid}:{controlId}".
/// </summary>
internal sealed class BridgeAgentElement
{
    ...
    public static bool IsDotnetId(string id) =>
        id.StartsWith("dotnet:", StringComparison.Ordinal);

    public static bool TryParseId(string id, out string pid, out int controlId)
    {
        pid = "";
        controlId = 0;
        if (!id.StartsWith("dotnet:", StringComparison.Ordinal)) return false;
        var parts = id.Split(':');
