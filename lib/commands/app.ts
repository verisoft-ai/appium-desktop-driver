...
export async function getWindowHandle(this: AppiumDesktopDriver): Promise<string> {
  ...
  const nativeWindowHandle = (await this.sendCommand('getProperty', {
    elementId: rootId,
  ...
  for (let i = 1; i <= SET_WINDOW_MAX_POLL_ATTEMPTS; i++) {
    if (!isNaN(handle)) {
      ...
      try {
        ...
        this.log.debug(
          `[setWindow] setRootElementFromHandle failed for handle 0x${handle.toString(16)}: ${err instanceof Error ? err.message : err}. Falling through to retry.`,
        );
      }
    } else {
    ...
    this.log.info(
      `Failed to locate window with name or handle '${nameOrHandle}'. Sleeping for ${POLL_INTERVAL_MS}ms and retrying... (${i}/${SET_WINDOW_MAX_POLL_ATTEMPTS})`,
    );
...
export async function switchToWindowByTitle(
  ...
  for (let i = 1; i <= SET_WINDOW_MAX_POLL_ATTEMPTS; i++) {
    ...
    this.log.info(
      `Failed to locate window with title '${title}'. Sleeping for ${POLL_INTERVAL_MS}ms and retrying... (${i}/${SET_WINDOW_MAX_POLL_ATTEMPTS})`,
    );
    await sleep(POLL_INTERVAL_MS);
  }
...
export async function changeRootElement(
  ...
  if (typeof pathOrNativeWindowHandle === 'number') {
    ...
    const elementId = (await this.sendCommand('setRootElementFromHandle', {handle: nativeWindowHandle})) as
      | string
      | null;
  ...
  if (isUwp) {
    ...
    for (let i = 1; deadline ? performance.now() < deadline : i <= 10; i++) {
      ...
      if (newHandles.length > 0) {
        this.log.debug(
          `Found ${newHandles.length} new window(s) since launch: ${newHandles.map((h) => `0x${h.toString(16).padStart(8, '0')}`).join(', ')}`,
        );
        try {
      ...
      this.log.info(
        `Failed to locate window of the app. Sleeping for ${POLL_INTERVAL_MS}ms and retrying... (attempt ${i})`,
      );
      await sleep(POLL_INTERVAL_MS);
    }
  } else {
    ...
    try {
      ...
      if (!result.focused && deadline && performance.now() < deadline) {
        this.log.info(
          'Attached to a window that cannot receive focus (likely a splash screen). Waiting for the main window...',
        );
      ...
      this.log.info(
        `PID-based window search failed (${err instanceof Error ? err.message : err}). Falling back to new-window detection.`,
      );
    }
    ...
    while (performance.now() < fallbackDeadline) {
      ...
      if (newHandles.length > 0) {
        this.log.debug(
          `Singleton-delegator fallback: found ${newHandles.length} new window(s): ${newHandles.map((h) => `0x${h.toString(16).padStart(8, '0')}`).join(', ')}`,
        );
        if (await this.attachToWindowHandles(newHandles)) {
          return;
        }
...
export async function waitForNewWindow(
  ...
  launcherPid: number,
  timeout: number,
): Promise<{handle: number; knownPids: number[]}> {
  ...
  while (performance.now() - start < timeout) {
    ...
    this.log.debug(
      `Waiting for the process window to appear... (${++attempts}/${Math.floor(timeout / POLL_INTERVAL_MS)})`,
    );
    await sleep(POLL_INTERVAL_MS);
  }
 ...
 */
export async function attachToWindowHandles(this: AppiumDesktopDriver, handles: number[]): Promise<boolean> {
  ...
  for (const hwnd of handles) {
    ...
    try {
      ...
      this.log.debug(
        `[attachToWindowHandles] elementFromHandle failed for 0x${hwnd.toString(16).padStart(8, '0')}: ${err instanceof Error ? err.message : err}`,
      );
      continue;
    }
    if (!candidateId) {
    ...
    const className = (await this.sendCommand('getProperty', {
    ...
    if (focusables && focusables.length >= 2) {
      this.log.info(
        `Attaching to window 0x${hwnd.toString(16).padStart(8, '0')} (${focusables.length} focusable descendants).`,
      );
      ...
      if (isNotNull) {
        const rootId = (await this.sendCommand('saveRootElementToTable', {})) as string;
        const nwh = Number(
          (await this.sendCommand('getProperty', {elementId: rootId, property: 'NativeWindowHandle'})) as string,
        );
...
export async function attachToApplicationWindow(
  ...
  const waitForAppLaunchMs =
    normalizeWaitForAppLaunchMs(this.caps['ms:waitForAppLaunch']) || POLL_INTERVAL_MS * MAX_POLL_ATTEMPTS;
  ...
  for (let i = 1; i <= 5; i++) {
    try {
      ...
      this.log.debug(
        `[attachToApplicationWindow] elementFromHandle failed for 0x${nativeWindowHandle.toString(16).padStart(8, '0')}: ${err instanceof Error ? err.message : err}`,
      );
      elementId = '';
    }
    ...
    if (!elementId) {
      for (const pid of knownPids) {
        const foundId =
          ((await this.sendCommand('findElement', {
            scope: 'children',
        ...
        if (foundId) {
          this.log.info(
            `Found window by ProcessId ${pid} (handle mismatch: Win32=0x${nativeWindowHandle.toString(16).padStart(8, '0')})`,
          );
          elementId = foundId;
          break;
        }
  ...
  if (!elementId) {
    throw new errors.UnknownError(
      `Window 0x${nativeWindowHandle.toString(16).padStart(8, '0')} did not appear in the UI Automation tree.`,
    );
  }
  ...
  if (isNotNull) {
    ...
    if (!focused) {
      try {
        ...
        this.log.debug(
          `[attachToApplicationWindow] focusElement failed (likely splash screen): ${err instanceof Error ? err.message : err}`,
        );
      }
    }
    ...
    if (focused) {
      try {
        ...
        if (!focusables || focusables.length < 2) {
          this.log.info(
            `Attached window has only ${focusables?.length ?? 0} focusable descendant(s); treating as a splash screen.`,
          );
          focused = false;
        }
      } catch (err) {
        this.log.debug(
          `[attachToApplicationWindow] splash-screen focusable-descendant probe failed, leaving focused as-is: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
...
export async function waitForMainWindow(
  ...
  try {
    ...
    this.log.debug(
      `[waitForMainWindow] Could not read splash screen handle, will just watch for staleness: ${err instanceof Error ? err.message : err}`,
    );
  }

  this.log.debug(
    `Splash screen handle: ${splashHandle != null ? `0x${splashHandle.toString(16).padStart(8, '0')}` : 'unknown'}. Polling for main window...`,
  );
  ...
  while (performance.now() < deadline) {
    ...
    try {
      ...
      this.log.debug(
        `[waitForMainWindow] Root element property read failed (treating as stale): ${err instanceof Error ? err.message : err}`,
      );
      rootStale = true;
    }
    ...
    if (rootStale) {
      this.log.info(
        `Splash screen closed (root element stale after ${attempt} polls). Re-attaching to main window via attachToApplicationWindow...`,
      );
    ...
    if (splashHandle != null) {
      ...
      if (newHandle) {
        this.log.info(
          `New window detected (0x${newHandle.toString(16).padStart(8, '0')}) alongside splash screen (attempt ${attempt}). Waiting for splash to close...`,
        );
  ...
  this.log.warn(
    'Deadline reached while waiting for splash screen to close. Session is attached to the splash screen window.',
  );
}
