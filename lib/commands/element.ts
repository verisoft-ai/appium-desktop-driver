...
export async function click(this: AppiumDesktopDriver, elementId: string): Promise<void> {
  ...
  if (!isMenuItem) {
    const focusCondition = andCondition(
      ...
      orCondition(propertyCondition('ControlType', 'Pane'), propertyCondition('ControlType', 'Window')),
    );
  ...
  try {
    const clickablePoint = (await this.sendCommand('getProperty', {elementId, property: 'ClickablePoint'})) as {
      x: number;
      y: number;
    };
    if (
      clickablePoint &&
      typeof clickablePoint === 'object' &&
      typeof clickablePoint.x === 'number' &&
      typeof clickablePoint.y === 'number'
    ) {
  ...
  this.log.debug(
    `click(${elementId}) controlType=${controlType || '?'} coords=(${coordinates.x},${coordinates.y}) menu=${isMenuItem}`,
  );
  ...
  if (isMenuItem) {
    ...
    try {
      const refreshed = (await this.sendCommand('getProperty', {elementId, property: 'ClickablePoint'})) as {
        x: number;
        y: number;
      };
      ...
      if (drift > 2) {
        this.log.debug(
          `click(${elementId}) popup settled: (${coordinates.x},${coordinates.y}) → (${refreshed.x},${refreshed.y}) — re-teleporting`,
        );
    ...
    if (afterMovePos) {
      ...
      if (cursorDrift > 2) {
        this.log.warn(
          `click(${elementId}) cursor drift: wanted (${coordinates.x},${coordinates.y}), got (${afterMovePos.x},${afterMovePos.y})`,
        );
      } else {
 ...
 */
export async function setFrame(this: AppiumDesktopDriver, id: null | number | Record<string, string>): Promise<void> {
  if (!this.isIEContext()) {
