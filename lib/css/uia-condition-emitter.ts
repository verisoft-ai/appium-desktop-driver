...
export class UiaConditionEmitter implements StrategyEmitter {
    ...
    private emitRule(rule: ParsedRule): string[] {
        if (rule.nested) {
            throw new errors.InvalidSelectorError('Combinators (descendant/child selectors) are not supported for css selector locators. Use a single compound rule.');
        }
        ...
        if (rule.classes.length > 1) {
            throw new errors.InvalidSelectorError('Only a single class token is supported in css selector (Windows elements have one ClassName).');
        }
        if (rule.classes.length === 1) {
    ...
    private formatAttribute(attr: ParsedAttribute): string {
        ...
        if (stringProperty) {
            if (attr.operator && attr.operator !== '=') {
                throw new errors.InvalidSelectorError(`Operator '${attr.operator}' is not supported in css selector — UIA property conditions only support exact match ('=').`);
            }
