...
import type {DesktopDriverConstraints} from './constraints';
...
import type {DesktopDriverServerClient} from './server/client';
...
export class AppiumDesktopDriver extends BaseDriver<DesktopDriverConstraints, StringRecord> {
  ...
  override getProxyAvoidList(): RouteMatcher[] {
    if (this.jwpProxyActive && this.chromedriver) {
      return CHROMEDRIVER_NO_PROXY;
    }
    return [];
  }
  ...
  override async createSession(
    ...
    try {
      ...
      if (this.caps.systemPort) {
        this.log.info(
          `systemPort capability (${this.caps.systemPort}) is ignored. AppiumDesktopDriver uses stdin/stdout IPC.`,
        );
      }

      // UIA server always starts. IEDriverServer starts lazily on first IE window switch.
      {
        const javaSwingLaunchPath =
          this.caps.javaSwing &&
          !!this.caps.app &&
          this.caps.app !== 'root' &&
          this.caps.app !== 'none' &&
        ...
        if (javaSwingLaunchPath) {
          ...
          this.caps.appArguments = this.caps.appArguments ? `${agentFlag} ${this.caps.appArguments}` : agentFlag;
        ...
        if (this.caps.javaSwing) {
          if (javaSwingLaunchPath) {
            ...
            await this.sendCommand('injectJavaAgent', {
              hwnd: Number(this.caps.appTopLevelWindow),
              jdkPath: this.caps.jdkPath,
            });
    ...
    if (this.caps.shouldCloseApp && this.caps.app && this.caps.app.toLowerCase() !== 'root') {
      try {
        if (this.caps['ms:forcequit'] === true) {
          ...
          if (isNotNull) {
            const processId = (await this.sendCommand('getProperty', {
              elementId: await this.sendCommand('saveRootElementToTable', {}),
              property: 'ProcessId',
            })) as string;
        ...
        this.log.debug(
          `[deleteSession] Postrun cleanup (process stop / window close) failed: ${err instanceof Error ? err.message : err}`,
        );
      }
    }
