...
export class IESession {
  ...
  private send(cmd: string, extra: Record<string, unknown> = {}): Promise<Record<string, unknown>> {
    return new Promise((resolve, reject) => {
      const seq = ++this.seq;
      this.pending.set(seq, {resolve, reject});
      this.proc.stdin!.write(JSON.stringify({seq, cmd, hwnd: this.hwnd, ...extra}) + '\n');
    });
  }
  ...
  async findElements(strategy: string, value: string): Promise<Array<{[key: string]: string}>> {
...
function strategyToCmd(strategy: string, multi: boolean, value: string): CmdSpec {
  switch (strategy) {
    ...
    default:
      throw new errors.InvalidArgumentError(`IE bridge does not support strategy: ${strategy}`);
  }
}
...
export function getIESession(sessionId: string): IESession {
  ...
  if (!s) {
    throw new Error(`No IE session: ${sessionId}`);
  }
  return s;
}
