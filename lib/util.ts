...
export async function getBundledFfmpegPath(driver: AppiumDesktopDriver): Promise<string | null> {
  ...
  try {
    ...
    await fs
      .unlink(zipPath)
      .catch((err) =>
        driver.log.debug(`ffmpeg: failed to remove temp zip '${zipPath}': ${err instanceof Error ? err.message : err}`),
      );
      ...
      const req = protocol.get(currentUrl, async (res) => {
        ...
        try {
          ...
          await fs
            ...
            .catch((unlinkErr) =>
              log.debug(
                `Failed to remove partial download '${fullFilePath}': ${unlinkErr instanceof Error ? unlinkErr.message : unlinkErr}`,
              ),
            );
          reject(err);
        }
