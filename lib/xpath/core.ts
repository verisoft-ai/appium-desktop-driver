...
import {PropertyCondition, TrueCondition, FalseCondition, AndCondition, OrCondition} from '../powershell/conditions';
import {Property, Int32Property, StringProperty, BooleanProperty} from '../powershell/types';
...
export async function processExprNode<T>(
  ...
  switch (exprNode.type) {
    ...
    case UNION:
      return [
        ...(await processExprNode<T>(exprNode.lhs, context, sendCommand)),
        ...(await processExprNode<T>(exprNode.rhs, context, sendCommand)),
      ];
    case FUNCTION_CALL:
...
async function executeStep(step: StepNode, context: XPathElement, sendCommand: SendCommandFn): Promise<XPathElement[]> {
  ...
  const condition =
    predicateConditions.length > 0 ? new AndCondition(nodeTestCondition, ...predicateConditions) : nodeTestCondition;
  ...
  if (step[OptimizeLastStep]) {
    const result = (await sendCommand('findElement', {scope, condition: conditionDto, contextElementId})) as
      | string
      | null;
    elIds = result ? [result] : [];
  } else {
  ...
  switch (nodeTest.type) {
    case NODE_NAME_TEST:
      ...
      try {
        ...
        log.debug(
          `Unknown ControlType '${nodeTest.name}' (${err instanceof Error ? err.message : err}), falling back to ClassName condition.`,
        );
    ...
    case NODE_NAME_TEST:
      ...
      // in their info dict so elIds above is empty; query contextId directly.
      {
        ...
        try {
          ...
          log.debug(
            `getProperty failed for unknown attribute '${nodeTest.name}' on element '${id}': ${err instanceof Error ? err.message : err}`,
          );
          return [''];
        }
      }
...
function isXPathElement(value: unknown): value is XPathElement {
  return (
    ...
    value !== null &&
    'type' in value &&
    ((value as any).type === 'root' || (value as any).type === 'found')
  );
}
