...
export async function handleFunctionCall<T>(
  ...
  switch (name) {
    ...
    case NAME: {
      ...
      const elId =
        element.type === 'found' && element.elementId
          ? element.elementId
    ...
    case TRANSLATE: {
      ...
      const [firstArgResult, secondArgResult, thirdArgResult] = await processArgs<string | XPathElement>(
        args[0],
        args[1],
        args[2],
      );
      ...
      if (isXPathElement(str)) {
        return ['' as T];
      }
      if (isXPathElement(from)) {
        return [str as T];
      }
...
function convertProcessedExprNodesToNumbers<T>(...arrayOfProcessedExprNodes: T[]): number[] {
  ...
  return arrayOfStrings.map((str) =>
    /^\s*(?<![\d.+-])[+-]?(?:\d*[.])?\d+(?![\d.])\s*$/.test(str) ? Number(str) : NaN,
  );
}
