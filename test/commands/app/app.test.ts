...
describe('getWindowHandle', () => {
  ...
  it('returns hex-formatted window handle', async () => {
    const driver = createMockDriver() as any;
    driver.sendCommand.mockResolvedValueOnce('root-id').mockResolvedValueOnce('12648430'); // 0x00C0FFEE
  ...
  it('pads handle to 8 hex digits', async () => {
    const driver = createMockDriver() as any;
    driver.sendCommand.mockResolvedValueOnce('root-id').mockResolvedValueOnce('1'); // 0x00000001
    const result = await getWindowHandle.call(driver);
...
describe('setWindow', () => {
  ...
  it('sets root element by window name when name is not numeric', async () => {
    ...
    driver.sendCommand.mockResolvedValueOnce('5.6.7').mockResolvedValueOnce(undefined);

    await setWindow.call(driver, 'Calculator');

    expect(driver.sendCommand).toHaveBeenCalledWith('findElement', expect.objectContaining({scope: 'children'}));
