...
import {APPIUM_SERVER, isProcess32Bit, launchMinimalOwnerDrawX86Externally, quitSession} from './helpers/session.js';
  ...
  it('windows: attachDotnetBridge routes through the x86 injector stub and makes the value readable', async () => {
    ...
    await driver.waitUntil(
        ...
        ((await driver.executeScript('windows: getPageSourceViaDotnetBridge', [{}])) as string).includes('Healthy'),
      {
        timeout: 10_000,
        interval: 500,
        timeoutMsg: 'Painted value never appeared after attachDotnetBridge on a 32-bit target',
      },
    );
  });
});
