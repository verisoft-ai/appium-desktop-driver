...
describe('.NET Bridge — ownerdraw gallery, full element coverage', () => {
    ...
    it('windows: attachDotnetBridge makes owner-drawn values readable', async () => {
        await driver.executeScript('windows: attachDotnetBridge', [{}]);
        await driver.waitUntil(
            async () => (await driver.getPageSource()).includes('Apple'),
    ...
    it('ListBox: exposes all 3 items and selectElement moves the real SelectedIndex', async () => {
        const items = await driver.$$('//BridgeListItem');
        expect(items.length).toBe(3);

        const banana = await driver.$('//BridgeListItem[contains(@Name,"Banana")]');
        ...
        const apple = await driver.$('//BridgeListItem[contains(@Name,"Apple")]');
    ...
    it('TreeView: collapsed node hides its children until expandElement runs', async () => {
        const childBefore = await driver.$$('//BridgeTreeNode[contains(@Name,"Child A1")]');
        expect(childBefore.length).toBe(0);

        const rootA = await driver.$('//BridgeTreeNode[contains(@Name,"Root A")]');
        ...
        const childAfter = await driver.$('//BridgeTreeNode[contains(@Name,"Child A1")]');
    ...
    it('Combo: opens a real popup window; selecting an item updates the closed value', async () => {
        const combo = await driver.$('//*[@AutomationId="Combo1"]');
        ...
        const green = await driver.$('//BridgeListItem[contains(@Name,"Green")]');
    ...
    it('ContextMenu: opens a real popup window; selecting an item updates the last-action value', async () => {
        const menu = await driver.$('//*[@AutomationId="ContextMenu1"]');
        ...
        await driver.switchToWindow(popup!.handle);
        const deleteItem = await driver.$('//BridgeListItem[contains(@Name,"Delete")]');
    ...
    it('ModalDialog: a second top-level window (same PID) is readable via the same bridge session', async () => {
        ...
        const status = await driver.$('//*[@AutomationId="DialogStatus"]');
