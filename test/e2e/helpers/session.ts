...
export async function launchJavaSwingFormExternally(): Promise<{proc: ChildProcess; hwnd: string}> {
  ...
  while (Date.now() < deadline) {
    try {
      hwnd = execSync(`powershell -Command "(Get-Process -Id ${pid} -ErrorAction Stop).MainWindowHandle"`, {
        stdio: ['ignore', 'pipe', 'ignore'],
      })
        .toString()
        .trim();
...
export async function createJavaSwingAttachSession(
  hwnd: string,
...
export const DEVEXPRESS_GRID_APP_PATH = resolve(
  TEST_APPS_DIR,
  'devexpress-grid-ownerdraw',
  'bin',
  'DevExpressGridOwnerDraw.exe',
);
...
export const MINIMAL_OWNERDRAW_APP_PATH = resolve(
  TEST_APPS_DIR,
  'minimal-ownerdraw-winforms',
  'bin',
  'x64',
  'Debug',
  'net472',
  'MinimalOwnerDraw.exe',
);
...
export const MINIMAL_OWNERDRAW_X86_APP_PATH = resolve(
  TEST_APPS_DIR,
  'minimal-ownerdraw-x86',
  'bin',
  'x86',
  'Debug',
  'net472',
  'MinimalOwnerDrawX86.exe',
);
...
export const NET8_WINFORMS_MINIMAL_APP_PATH = resolve(
  TEST_APPS_DIR,
  'net8-winforms-minimal',
  'bin',
  'Debug',
  'net8.0-windows',
  'Net8WinformsMinimal.exe',
);
...
export const DEVEXPRESS_ELEMENTS_GALLERY_APP_PATH = resolve(
  TEST_APPS_DIR,
  'devexpress-elements-gallery',
  'bin',
  'DevExpressElementsGallery.exe',
);
...
export const WPF_DEVEXPRESS_LIGHTWEIGHT_APP_PATH = resolve(
  TEST_APPS_DIR,
  'wpf-devexpress-lightweight',
  'bin',
  'WpfDevExpressLightweight.exe',
);
...
export async function createDotnetBridgeAttachSession(
  hwnd: string,
  extraCaps?: Record<string, unknown>,
): Promise<Browser> {
  const driver = await remote({
